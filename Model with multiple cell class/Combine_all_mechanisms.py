...
for i in range(num_x-2,len(sheet.face_df)):     # These are the indices of the top layer.
    ...
    sheet.face_df.loc[i, 'timer'] = 0.15    # Add a timer for STB units, this is used for STB-E later.
...
# cell class transition rules implementation starts here.
    # For all STB units, decrease the timer by dt. If timer <0, then it extrudes.
    STB_units = sheet.face_df.index[sheet.face_df['cell_class'] == 'STB'].tolist()
    for unit in STB_units:
        if sheet.face_df.loc[unit, 'timer'] < 0:
            if rng.random() < 0.1:  # 10% probability to extrude.
                sheet.face_df.loc[unit,'cell_class'] = 'E'  # Mark the cell as extruding.
                sheet.face_df.loc[unit,'timer'] = 0.15
                detach(sheet, geom, unit)
        else:
...
hdf5.save_datasets('three_mech.hdf5', sheet)
...
with imageio.get_writer('three_mech.mp4', fps=15, format='ffmpeg') as writer:
