...
# Generate the initial cell sheet.
print('\n We start with a petri dish geometry. ')
num_x = 16
num_y = 6
...
print(f'There are {total_cell_num} total cells; assigning them into "S" or "STB" class with 0.5 probability. ')
cell_class_list = ['STB', 'S']
for i in sheet.face_df.index:
    sheet.face_df.loc[i,'cell_class'] = rng.choice(cell_class_list)
...
cell0_class = sheet.face_df.loc[0,'cell_class']
print(f'Cell 0 is in class: "{cell0_class}" at t=0.')

while t <= t_end:
    ''''
    # Select all cells in "S"
    ...
    for cell in S_cells:
        ...
        if cell_fate_roulette <= 0.01:
            sheet.face_df.loc[cell, 'cell_class'] = 'G2'
            print(f'Cell {cell} enter "G2" at time {t}. ')
    ...
    for cell in G2_cells:
        if sheet.face_df.loc[cell, 'timer'] < 0:
            ...
            print(f'Cell {cell} enter "M" at time {t}. ')
        else:
...
# For all cells in "M", divide the cell with no orientation preference. Then cells becomes "G1".
    ...
    for cell in M_cells:
        ...
        if cell == 0:
            print(f'Cell 0 enter "G1" at time {t}. ')
    ...
    for cell in G1_cells:
        if sheet.face_df.loc[cell, 'timer'] < 0:
            ...
            if cell == 0:
                print(f'Cell 0 enter "S" at time {t}. ')
        else:
            sheet.face_df.loc[cell, 'timer'] -= dt
    '''
...
geom.update_all(sheet)
fig, ax = sheet_view(sheet)
ax.set_title("unrealistic configuration simulated")
plt.show()
